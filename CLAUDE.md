...
**Şu anki faz**: Faz 2 — plan onaylandı (A1+B4+C1, [ADR 0008](docs/decisions/0008-statevector-cekirdek-mimarisi.md)). Çekirdek yazıldı, C-sim ile doğrulandı (fidelity ≥0,99997, n=8/12/16) ve **sentezlendi**: 7,195 ns (100 MHz), BRAM %66, DSP %16, FF %46, **LUT %84**, gecikme 6.948.095 çevrim = **0,0695 sn**. SC-002/SC-003 geçti, NC-4 kapandı. CPU (Aer ~58 ms) hâlâ 1,2× hızlı, **hızlanma iddiası yok**. Vitis 2025.2 **WSL/Ubuntu altında kurulu ve çalışıyor** — Windows'ta Device Guard engellemişti ([SK-05](docs/risk-register.md)), taşınmanın ölçümü bozmadığı Tur 15 yeniden üretilerek kanıtlandı. Kurulum ve sentez komutları: [runbook](docs/runbooks/vitis-hls-kurulum.md). ⚠️ LUT payı %37'den %16'ya düştü; paralellik kararı bu kısıt altında verilecek. Durum ve sıradaki iş: [tasks.md SIRADAKİ](specs/002-fpga-statevector-cekirdegi/tasks.md).
