...
- [X] T026 [US1] `agent/board.py` — register haritası sabitleri (`xqir_kernel_hw.h`'den birebir): `AP_CTRL 0x0000`, `P 0x0048`, `BEKLENEN_DEGER 0x0050`, `BEKLENEN_DEGER_CTRL 0x0054`, `COS_BETA 0x0020` (3), `SIN_BETA 0x0030` (3), `PHASES 0x1000` (816), `COST 0x2000` (272); MMIO uzunluğu `0x10000`. Eleman adresi: `taban_ofset + 4*n`
- [X] T027 [US1] `agent/board.py` — iki erişim yolu: A (`Overlay`) ve B (`Bitstream(...).download()` + `MMIO(taban, 0x10000)`). **Varsayılan T007'nin sonucuna göre seçilir**, diğeri yedek olarak kalır. Taban adres **sabit** olarak tutulur (`board.AXI_TABAN = 0x43C00000`) ve A yolunda `ip_dict['phys_addr']` ile **çapraz doğrulanır**. ⚠️ Görev metni README'den okumayı söylüyordu; düzyazı ayrıştırmak kırılgan, sözleşme + çalışma zamanı çapraz kontrolü daha sağlam. Uyuşmazlıkta istisna atılır
