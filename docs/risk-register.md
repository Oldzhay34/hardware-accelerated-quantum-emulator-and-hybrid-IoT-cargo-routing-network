...
| **DT-01** | Kart arızalı / boot etmiyor · `K-01` | D | **Y** | İlk güç verişte DONE LED'i yanmıyor **veya** SD karttan boot logu seri portta akmıyor | **H1** (20 Eyl) | Kart boot ediyor, Jupyter ağdan açılıyor, örnek overlay yükleniyor | H2'de 2. SD kart + farklı adaptör; tutmazsa Senaryo B kimliği, Faz 5 düşer | Olcay | 🟢 **BÜYÜK ÖLÇÜDE KAPALI** (2026-09-15) — COM3'ten (FTDI FT2232H, TULB seri no) canlı boot logu alındı: *"PYNQ Linux, based on Ubuntu 18.04 / pynq login: xilinx (automatic login)"*. Kalan iki ölçüt (Jupyter ağdan açılıyor, örnek overlay yükleniyor) henüz doğrulanmadı. |
